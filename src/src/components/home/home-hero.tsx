...
export function HomeHero({ kbName, docCount }: HomeHeroProps) {
  return (
    <section className="rounded-[1.5rem] border border-border bg-muted/50 p-5 shadow-[0_24px_80px_rgba(0,0,0,0.28)] backdrop-blur-xl">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2 text-sm text-foreground/90">
          <span className="rounded-full border border-border bg-secondary px-3 py-2">
            知识库：{kbName}
          </span>
          <span className="rounded-full border border-border bg-secondary px-3 py-2">
            资料：{docCount}
